...
The output canvas is the bbox size multiplied by the global 750px scale. This
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Global source-to-750px scale used for output size and scaled_bbox metadata.",
    )
    parser.add_argument("--z-index", type=int, default=0, help="Layer z-index for manifest output.")
...
def main() -> int:
    ...
    output_width = max(1, int(round(args.width * args.scale)))
    output_height = max(1, int(round(args.height * args.scale)))
    if asset.size != (output_width, output_height):
        try:
            resampling = Image.Resampling.LANCZOS
        except AttributeError:
            resampling = Image.LANCZOS
        asset = asset.resize((output_width, output_height), resampling)
    ...
    if args.manifest:
        ...
        scaled_bbox = {
            "x": round(args.x * args.scale, 4),
            "y": round(args.y * args.scale, 4),
            "width": round(args.width * args.scale, 4),
            "height": round(args.height * args.scale, 4),
        }
        update_manifest(
            Path(args.manifest),
            {
