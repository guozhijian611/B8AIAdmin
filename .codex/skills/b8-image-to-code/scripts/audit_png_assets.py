...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "[{\"asset\":\"assets/icons/a.png\",\"scaled_bbox\":{\"width\":40,\"height\":40}}], "
...
def load_manifest(path: str | None) -> dict[str, tuple[int, int]]:
    ...
    if isinstance(data, list):
        for item in data:
            key = item.get("path") or item.get("asset")
            if not key:
                continue
            bbox = item.get("scaled_bbox") or item.get("source_bbox") or item
            if "width" not in bbox or "height" not in bbox:
                continue
            result[str(key)] = (int(round(float(bbox["width"]))), int(round(float(bbox["height"]))))
...
def manifest_size(manifest: dict[str, tuple[int, int]], path: Path) -> tuple[int, int] | None:
    ...
    for key, size in manifest.items():
        if path.as_posix().endswith(key) or key.endswith(path.as_posix()) or key.endswith(path.name):
            return size
    return None
