...
- `art-*` 图表组件（`src/components/core/charts/` 已有 bar/line/ring/radar/scatter/k-line/gauge 等）封装成统一约定的「大屏组件」：输入 `{ option/props, data }`，输出渲染。
...
| 取值类型 | 数据源 | 返回结构 | 适用组件 |
| --- | --- | --- | --- |
| `table_raw` 表原始行 | MySQL | 返回明细 `rows`，字段来自「返回字段」和「计算字段」。 | 表格、排行榜、折线图、柱状图、散点图、K线图、仪表盘、图片轮播。 |
| `table_count` 表计数 | MySQL | 返回 `rows[0].total`，`total` 同步为计数值。 | 指标卡、仪表盘、总量统计、告警数量。 |
| `table_aggregate` 表聚合 | MySQL | 按维度字段分组，固定输出 `label`，再输出一个或多个聚合指标。 | 柱状图、环图、雷达图、趋势图、多指标对比、仪表盘。 |
| `http_passthrough` HTTP 透传 | HTTP | 若接口返回 `rows` 则直接使用；否则取 `data`，数组转多行，对象转单行。 | 外部系统指标、第三方接口、已聚合好的业务数据、K线图、仪表盘。 |
...
- 编辑器「适应窗口」按容器宽高计算等比缩放，最大不超过 100%。画布内部仍保留设计稿坐标，例如 1920 × 1080；外层使用缩放后的宽高占位，避免缩小后滚动区域仍按原尺寸计算。
- 运行页使用大屏背景配置里的 `fit_mode`：`contain` 完整显示、`cover` 裁切铺满、`stretch` 非等比拉伸。
- 编辑器是设计态，需要保留滚动和操作空间；运行页是展示态，会把画布绝对定位到窗口中。
...
| 前端 | `DraggableItem.vue`（封装 `vue3-draggable-resizable`）+ `widgets/` 注册表，复用 `art-*` 图表（柱/折线/横向柱/K线/仪表盘/环形/雷达/散点 + 单值指标 / 表格）并提供 CSS 装饰边框。 |
...
- 已完成：主题预设、背景图与图片适配；横向柱图 / 双向对比柱图 / K线图 / 仪表盘 / 雷达图 / 散点图；图片轮播；点位地图；CSS 装饰边框 / 扫描线装饰；新增图表字段映射；表格列宽 / 对齐 / 字段别名展示；编辑器复制 / 粘贴 / 撤销 / 重做基础操作；编辑器基础多选、批量复制 / 删除 / 置顶 / 置底 / 对齐 / 分布 / 组合 / 取消组合 / 整体拖拽 / 整体缩放；编辑器缩放占位自适应；图层面板基础排序；装饰组件运行时免取数；查询模板条件分组 / OR 组合；公开运行时轮询下限、IP / 大屏 / 创建人限流、Redis 原子锁优先的互斥回源、stale 缓存兜底、运行指标与缓存命中率观测。
- 未完成：漏斗图、热力图、时间轴等更多垂直领域图表，以及更复杂的装饰组件按实际大屏场景继续扩展。
