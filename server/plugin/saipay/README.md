...
2. 前台让用户在支付宝收款码和微信收款码里二选一，并只展示选中的收款码。
3. 用户付款后点击“我已付款”，前端提交 `pay_channel` 标记实际收款渠道，订单状态变为 `待确认`，系统给 `manual_scan_notice_emails` 配置的管理员邮箱发送通知。
4. 管理员在后台订单列表核对到账后点击“确认到账”，订单变为已支付并触发 `saipay.order.paid` 事件；如果核对未到账，点击“驳回付款”并填写驳回备注，订单回到未支付。
...
4. **人工支付风控**：人工扫码支付不会自动校验资金到账，必须由管理员核对后点击“确认到账”；未查到账、金额不一致或渠道不匹配时应点击“驳回付款”，并填写备注。
